...
int aiMove() {
	...
	return miniMaxAlfaBeta(board, MAX_DEPTH, 0 - INT_MAX, INT_MAX, COMPUTER)[1];
}


array<int, 2> miniMaxAlfaBeta(vector<vector<int> >& b, unsigned int d, int alf, int bet, unsigned int p) {
	...
	if (p == COMPUTER) { 
		...
		for (unsigned int c = 0; c < NUM_COL; c++) { 
			if (b[NUM_ROW - 1][c] == 0) { 
				...
				int score = miniMaxAlfaBeta(newBoard, d - 1, alf, bet, PLAYER)[0];
		...
		for (unsigned int c = 0; c < NUM_COL; c++) {
			if (b[NUM_ROW - 1][c] == 0) {
				vector<vector<int> > newBoard = copyBoard(b);
				makeMove(newBoard, c, p);
				int score = miniMaxAlfaBeta(newBoard, d - 1, alf, bet, COMPUTER)[0];
				if (score < moveSoFar[0]) {
					moveSoFar = { score, (int)c };
				}
...
int main(int argc, char** argv) {
	cout << "Connect Four SerialAlfaBeta" << endl;
	printProjectInfo();
